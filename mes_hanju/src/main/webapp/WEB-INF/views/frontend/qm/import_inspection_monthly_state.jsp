    ...
    <div class="wrapper">
		...
		<div class="content-wrapper">
            <section class="content-header">
                <h1>
					품질관리 <small> 수입검사진행현황(업체,기간별)</small>
                </h1>
                <ol class="breadcrumb">
                    <li><a href="#"><i class="fa fa-dashboard"></i> 품질관리 </a></li>
                    <li class="active">수입검사진행현황(업체,기간별)</li>
                </ol>
            </section>
...
function fnSearchInspGrid() {
	...
	var page_url = "/frontend/qm/import_inspection_monthly_state_select"
	+ "?cmdat=" + encodeURIComponent($("#m_year").val())
	 + "&werks=" + encodeURIComponent($("#m_wkctr").val());
...
function fnLoadInspGrid() {
	$("#grid_role").jqGrid({
		...
		colNames : ['업체코드', '업체명',
		            ...
		            'h12','검사수량', '합격수량', '합격율(%)'],
		colModel : [
			{name : 'lifnr',index : 'lifnr',width : 130,sorttype : "text",sorttype:'text'},
			{name : 'name1',index : 'name1',width : 80,sorttype : "text",	sortable : true},
...
// 				var onepass_rate = 0; 
// 				var twopass_rate = 0; 
// 				var threepass_rate = 0; 
// 				var fourpass_rate = 0;
// 				var fivepass_rate = 0;
// 				var sixpass_rate = 0; 
// 				var sevenpass_rate = 0; 
// 				var eightpass_rate = 0; 
// 				var ninepass_rate = 0;
// 				var tenpass_rate = 0; 
// 				var elevenpass_rate = 0;
// 				var twelvepass_rate = 0;
				...
				test.lifnr = '-';
				test.name1 = '합계 (' + data.rows.length + ')';
				...
				test.onepass_rate    = ((onenow_pass_cnt/(onenow_done_cnt==0?1:onenow_done_cnt))*100).toFixed(1);
				test.twopass_rate    = ((twonow_pass_cnt/(twonow_done_cnt==0?1:twonow_done_cnt))*100).toFixed(1);
				test.threepass_rate  = ((threenow_pass_cnt/(threenow_done_cnt==0?1:threenow_done_cnt))*100).toFixed(1);
				test.fourpass_rate   = ((fournow_pass_cnt/(fournow_done_cnt==0?1:fournow_done_cnt))*100).toFixed(1);
				test.fivepass_rate   = ((fivenow_pass_cnt/(fivenow_done_cnt==0?1:fivenow_done_cnt))*100).toFixed(1);
				test.sixpass_rate    = ((sixnow_pass_cnt/(sixnow_done_cnt==0?1:sixnow_done_cnt))*100).toFixed(1);
				test.sevenpass_rate  = ((sevennow_pass_cnt/(sevennow_done_cnt==0?1:sevennow_done_cnt))*100).toFixed(1);
				test.eightpass_rate  = ((eightnow_pass_cnt/(eightnow_done_cnt==0?1:eightnow_done_cnt))*100).toFixed(1);
				test.ninepass_rate   = ((ninenow_pass_cnt/(ninenow_done_cnt==0?1:ninenow_done_cnt))*100).toFixed(1);
				test.tenpass_rate    = ((tennow_pass_cnt/(tennow_done_cnt==0?1:tennow_done_cnt))*100).toFixed(1);
				test.elevenpass_rate = ((elevennow_pass_cnt/(elevennow_done_cnt==0?1:elevennow_done_cnt))*100).toFixed(1);
				test.twelvepass_rate = ((twelvenow_pass_cnt/(twelvenow_done_cnt==0?1:twelvenow_done_cnt))*100).toFixed(1);
...
// 	console.log('excelFileDownload('+gridName+')');
	...
	if( gridName=='grid_role' ) {
		fileName = '수입검사진행현황(업체,기간별).xlsx';
		sheetTitle = '수입검사진행현황(업체,기간별)';
		sheetName = '수입검사진행현황(업체,기간별)';
	}
