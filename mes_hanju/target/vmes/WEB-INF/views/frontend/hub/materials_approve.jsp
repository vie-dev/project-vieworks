...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-dashboard"></i> SCM</a></li><li class="active">자재승인</li>
      </ol>
    </section>
					...
					<div class="box box-success box-solid">
						<div class="box-header with-border">
							...
							<div class="box-tools pull-right">
								<button type="button" id="btn_search" class="btn btn-primary btn-sm" onclick="fnSearchReqMastGrid();">조회</button>	
								...
								<div class="col-sm-2">
									<div class="form-group">
										...
										<input type="radio" name="rdo_req_type" class="flat-red" value="" checked>
											전체 </label> 
										<label> <input type="radio" name="rdo_req_type" class="flat-red" value="Y"> 
											승인 </label> 
										<label> <input type="radio" name="rdo_req_type" class="flat-red" value="N"> 
											미승인</label>
									</div>
								</div>
								<div class="col-sm-2">
									<div class="form-group">
										...
										<label> <input type="radio" name="rdo_plant_type" class="flat-red" value="" checked>
											전체 </label> 
										<label> <input type="radio" name="rdo_plant_type" class="flat-red" value="1110"> 
											의료 </label> 
										<label> <input type="radio" name="rdo_plant_type" class="flat-red" value="1210"> 
											광 </label>
									</div>
								</div>
							</div>
								...
								<div id="dv_grid_req" class="box-body">
									<div class="col-md-6">
										<div id="dv_grid_req_sernr" class="box-body">
											<button type="button" class="btn bg-maroon btn-sm grid_attach_down" onclick="excelFileDownload('grid_req_sernr');" data-selfilepath="/upload" data-selfilename="test.xlsx" style="margin-bottom: 5px;">
									...
									<div class="col-md-6">
										<div id="dv_grid_req_bdmng" class="box-body">
											<button type="button" class="btn bg-maroon btn-sm grid_attach_down" onclick="excelFileDownload('grid_req_bdmng');" data-selfilepath="/upload" data-selfilename="test.xlsx" style="margin-bottom: 5px;">
									 			<span class="glyphicon glyphicon-download-alt" aria-hidden="true">수량관리대상</span>
											</button>
											<button class="btn btn-warning btn-sm fileinput-button" style="margin-bottom: 5px;"> 
												<span>Excel 일괄처리</span> 
												<input id="sernrfileupload" type="file" name="files[]" multiple>
											</button>
<!-- 											<button type="button" id="btn_search" 	class="btn btn-success btn-sm" onclick="test();" style="margin-bottom: 5px;">Excel 일괄처리</button> -->
...
function fnLoadReqMastGrid(){
	...
	$("#grid_req").jqGrid({
		datatype: "json",
		colNames:['승인여부','요청번호','플랜트', '플랜트','요청자','요청일자'],
		colModel:[
			{name:'grantor_status',index:'grantor_status', width:20, sorttype:"text"},
	        {name:'reqm_matnr_key',index:'reqm_matnr_key', width:20, sorttype:"text"},
	        {name:'werks',index:'werks', width:20, sorttype:"text", hidden:true},
	        {name:'werks_nm',index:'werks_nm', width:20, sorttype:"text"},
...
function fnLoadReqDet1Grid() {
	...
	$("#grid_req_sernr").jqGrid({
		datatype: "json",
		colNames:['요청번호','요청번호순번','reqd_matnr_key','자재코드','자재명','Spec','location','From창고재고수량 (1100)','등급','요청수량', '시리얼번호', '단위', '요청자', '플랜트'],
		colModel:[
			...
			{name:'wrkst',index:'wrkst', hidden:true},
			{name:'location',index:'location', hidden:true},
			{name:'labst',index:'labst', hidden:true},
			{name:'grade_nm',index:'grade_nm'},
...
function fnLoadReqDet2Grid() {
	...
	$("#grid_req_bdmng").jqGrid({
		datatype: "json",
		colNames:['요청번호','요청번호순번','reqd_matnr_key','자재코드','자재명','Spec','location','From창고재고수량 (1100)','등급','요청수량', '승인수량','단위','요청자','플랜트'],
		colModel:[
			{name:'reqm_matnr_key',index:'reqm_matnr_key'},
			{name:'reqm_sub_key',index:'reqm_sub_key', hidden:true},
			{name:'reqd_matnr_key',index:'reqd_matnr_key', hidden:true},
...
//  	console.log("jsonStr_1 ::: " + jsonStr_1);
	...
	$.ajax({
	    ...
	    success:function(data, textStatus, jqXHR){
	    	if(data.status == "200") {
	    		if(data.e_return == "S") {
	    			fnMessageModalConfirm("Notification(HUB)", "정보를 저장하였습니다. <br/> *****sap 전송 결과***** <br/> e_return : " 
			    			+ data.e_return + "<br/>e_message : "+ data.e_message);	
	    		} else {
	    			fnMessageModalConfirm("Notification(HUB)", "정보 저장에 실패했습니다. <br/> *****sap 전송 결과***** <br/> e_return : " 
			    			+ data.e_return + "<br/>e_message : "+ data.e_message);
	    		}
...
// 		if( mrow.hidden==false && getColNames[idx]!='' ) {
		if( getColNames[idx]!='' ) {
// 		if( (getColNames[idx] == 'reqm_matnr_key')
// 			|| (getColNames[idx] == 'matnr')
// 			|| (getColNames[idx] == 'maktx')
// 			|| (getColNames[idx] == 'grade_nm')
// 			|| (getColNames[idx] == 'request_bdmng')
// 			|| (getColNames[idx] == 'serial')
// 			) {
	...
	var gridData_tmp = [];
	if(gridName == 'grid_req_bdmng') {
		gridData_tmp = $("#grid_req_bdmng").getRowData();
	} else {
		gridData_tmp = $("#grid_req_sernr").getRowData();
	}
	
	var gridData;
	//재고수량 데이터 추가
	if(gridData_tmp.length != 0) {
		gridData = addLabst(gridData_tmp);	
	} else {
	...
	if(gridCols != null && gridCols.length > 0){
		...
		for(var i=0; i<gridCols.length; i++){
			if ((gridCols[i].name == 'reqm_matnr_key') ||
				(gridCols[i].name == 'matnr') ||
				(gridCols[i].name == 'maktx') ||
				(gridCols[i].name == 'wrkst') ||
				(gridCols[i].name == 'location') ||
				(gridCols[i].name == 'labst') ||
				(gridCols[i].name == 'request_bdmng') ||
				(gridCols[i].name == 'serial'))
				{
...
// 		var params_detail = {};
// 		params_detail.matnr = gridData[i].matnr; 
		werks = gridData[i].werks;
		reqm_matnr_key = gridData[i].reqm_matnr_key;
		
// 		params.push(params_detail);
	}
		
		
	var page_url = "/sap_master/addLabst"
// 		 + "?gridData=" + encodeURIComponent(JSON.stringify(params));
// 		 + "&werks=" + encodeURIComponent(werks);
		 + "?reqm_matnr_key=" + encodeURIComponent(reqm_matnr_key)
		 + "&werks=" + encodeURIComponent(werks);
	$.ajax({
	    url: page_url,
	    type: "POST",
	    ...
	    success:function(data, textStatus, jqXHR){
	    	if(data.status == "200") {
	    		labst_data = data.rows;
	    		success_flag = true;
	    	} else {
	    		fnMessageModalAlert("Notification(SCM)", "SAP 자재조회 실패");
	    	}
	    },
	    error: function(jqXHR, textStatus, errorThrown){
	    	fnMessageModalAlert("Notification(SCM)", "SAP 자재조회 실패");	
	...
	if(success_flag == true) {
		for(var i=0 ; i<gridData.length ; i++) {
			for(var k=0 ; k<labst_data.length ; k++) {
				if(gridData[i].matnr == labst_data[k].matnr) {
					if(labst_data[k].lgort == '1100') {
						var labst = Number(labst_data[k].labst).toFixed(0);
						gridData[i].labst = labst;
					}
				}	
			}
		}
...
// 	console.log('fnSernrLoadFileHandler()');
	var page_url = "/file/req_import_upload";
	...
	$('#sernrfileupload').fileupload({
		...
		var gridData = $("#grid_req_bdmng").getRowData();
		
		for(var i=0 ; i<gridData.length ; i++) {
			for(var k=0 ; k<data.result.rows.length ; k++) {
				if(gridData[i].matnr == data.result.rows[k].matnr) {
					var sernr = Number(data.result.rows[k].sernr).toFixed(0);
					gridData[i].serial = sernr;
				}	
			}
		}

		$('#grid_req_bdmng').jqGrid('setGridParam', { datatype: 'local', data: gridData }).trigger('reloadGrid');
